# Description: Implementation of Q-learning algorithm for static target task

import pygame
import numpy as np
from env_static import Environment
import pandas as pd
import time
...
def train_agent(agent, env, num_episodes, max_steps, save=True, render=False):
    ...
    if save:
        # create folder for results "results/qlearning_static/train/"
        results_dir = os.path.join(current_dir, "results/qlearning_static/train/")
        if not os.path.exists(results_dir):
            os.makedirs(results_dir)

        # save results to csv file
        train_total_rewards_steps.to_csv(
            f'{results_dir}qlearning_static_train_{datetime.now().strftime("%Y%m%d-%H%M%S%f")}.csv',
            index=False,
            header=True,
        )
...
def test_agent(agent, env, num_episodes, max_steps, save=True, render=False):
    ...
    # Save the results to a CSV file if save is True, format csv file name with method and task and timestamp
    if save:
        # create folder for results "results/qlearning_static/test/"
        results_dir = os.path.join(current_dir, "results/qlearning_static/test/")
        if not os.path.exists(results_dir):
            os.makedirs(results_dir)

        # save results to csv file
        test_total_rewards_steps.to_csv(
            f'{results_dir}qlearning_static_test_{datetime.now().strftime("%Y%m%d-%H%M%S%f")}.csv',
            index=False,
            header=True,
        )
...
def plot_results(results, type, show=False, save=False):
    ...
    directory = os.path.join(current_dir, f"results/qlearning_static/plots/{type}/")
